...
func (ctx *context) testProgs(entries []*prog.LogEntry, duration time.Duration, opts csource.Options) (
	...
	command := fmt.Sprintf("%v -executor %v -arch=%v -os=%v -cover=0 -procs=%v -repeat=%v"+
		" -sandbox %v -threaded=%v -collide=%v %v",
		inst.execprogBin, inst.executorBin, ctx.cfg.TargetArch, ctx.cfg.TargetOS, opts.Procs, repeat,
